...
local function image_wallpaper(shape)
   ...
   local filename =
      string.sub(wallpaper.value or ('file://' .. gfs.get_configuration_dir() .. '/assets/wallpaper.png'), 8)
...
local function wallpaper_for_screen(s)
   ...
   local raw_widget = wallpaper.type == 'color' and color_wallpaper(shape) or image_wallpaper(shape)
