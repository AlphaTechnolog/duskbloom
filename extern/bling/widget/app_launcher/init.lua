  ...
  app:connect_signal(
    ...
    function(_self, lx, ly, button, mods, find_widgets_result)
      if button == 1 then
        local app = _self
        if
          self._private.active_widget == app or not self.select_before_spawn
        then
          app.spawn()
        else
...
local function page_backward(self, direction)
  ...
  for index, entry in pairs(self._private.matched_entries) do
    ...
    if
      index > min_app_index_to_include and index <= max_app_index_to_include
    then
      self._private.grid:add(create_app_widget(self, entry))
    end
  end
...
function app_launcher:hide()
  ...
  if animation ~= nil then
    ...
    local turn_off_on_anim_x_end = (anim_x_duration >= anim_y_duration) and true
      or false
...
local function new(args)
  ...
  args.skip_empty_icons = args.skip_empty_icons ~= nil and args.skip_empty_icons
    or false
  ...
  ret._private.prompt = prompt({
    ...
    changed_callback = function(text)
      ...
      if
        ret._private.search_timer ~= nil and ret._private.search_timer.started
      then
